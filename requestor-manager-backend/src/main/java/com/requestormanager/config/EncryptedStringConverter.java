...
/**
 * Transparent AES-256-GCM encryption at rest for sensitive string columns — currently the
 * per-subscription introspection client secret on {@code SubscriptionCredential}, which grants the
 * ability to introspect tokens at a requestor's identity provider and so must not sit in the
 ...
 * The {@code enc:v1:} prefix lets reads distinguish encrypted values from legacy plaintext rows, so
 * existing plaintext stays readable (passthrough) and is re-written encrypted on the next save. No
 * backfill migration is required; rows convert as they are touched.
 *
 * <p>A 36-character Keycloak secret encodes to roughly 95 characters, so the existing
 * {@code varchar(255)} column has ample headroom and needs no widening.
 *
 * <p><b>Key material:</b> a 32-byte AES key derived via SHA-256 from
 * {@code REQUESTOR_MANAGER_ENCRYPTION_KEY} if present, otherwise from the existing required secret
 * {@code KEYCLOAK_CLIENT_SECRET}. Prefer setting the dedicated key: falling back couples encryption
 * to an OAuth client secret, so rotating that client would leave stored secrets undecryptable.
 * Decryption fails open (the raw stored value is returned and a warning logged), so a key change
 * degrades to "credentials must be re-provisioned" rather than a startup failure.
 *
 * <p><b>Key availability timing:</b> JPA {@link AttributeConverter}s are not Spring beans, so a
 * Spring {@link Component} ({@link KeyHolder}) captures the resolved property into a static field
 * during context startup via {@link PostConstruct}. Converts only happen during actual persistence
 * operations, which occur after context initialization, so the key is present. As a defensive
 * fallback, the key is read lazily from {@code System.getenv} on first use.
 */
@Converter
public class EncryptedStringConverter implements AttributeConverter<String, String> {
    ...
    /**
     * Spring component whose sole job is to lift the resolved key property (env or yaml, with
     * relaxed binding) into the static holder so the non-managed converter can reach it.
     */
    @Component
    static class KeyHolder {
        KeyHolder(@Value("${requestor-manager.encryption-key:${keycloak.client-secret:}}") String key) {
        ...
        void ready() {
            if (configuredKeySource == null || configuredKeySource.isBlank()) {
                log.warn("EncryptedStringConverter: no requestor-manager.encryption-key or "
                        + "keycloak.client-secret resolved; will fall back to environment variables "
                        + "at convert time.");
            }
        }
    }
    ...
    private SecretKey key() {
        ...
        if (source == null || source.isBlank()) {
            source = System.getenv("REQUESTOR_MANAGER_ENCRYPTION_KEY");
        }
        if (source == null || source.isBlank()) {
            source = System.getenv("KEYCLOAK_CLIENT_SECRET");
        }
        if (source == null || source.isBlank()) {
            throw new IllegalStateException(
                    "No encryption key available: set REQUESTOR_MANAGER_ENCRYPTION_KEY or "
                            + "KEYCLOAK_CLIENT_SECRET (requestor-manager.encryption-key / keycloak.client-secret).");
        }
        try {
