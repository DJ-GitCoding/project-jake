...
const Header = () => {
  ...
  const { t } = useT();
  const { keycloakAdminUrl, appVersion } = useConfig();
  ...
  const handleLogout = () => {
    setMenuOpen(false);
    // Server destroys the session cookie and redirects to /login.
    logout();
  };
  ...
  const navItems = [
    { to: '/dashboard', icon: 'bi-house', label: t('header.nav.home'), show: true },
    { to: '/rdap-requests', icon: 'bi-clock-history', label: t('header.nav.requests'), show: true },
    { to: '/data-holders', icon: 'bi-database-gear', label: t('header.nav.dataHolders'), show: isAdmin },
    { to: '/logs', icon: 'bi-journal-text', label: t('header.nav.logs'), show: isAdmin },
        ...
        <div className="container-fluid px-3 px-md-4 d-flex align-items-center justify-content-between">
          ...
          <Link to="/dashboard" className="navbar-brand d-flex align-items-center gap-2 mb-0">
            ...
            <div className="d-flex flex-column lh-1">
              <span className="fw-bold text-primary" style={{ fontSize: '1.1rem' }}>Jaddar</span>
            ...
            <div className="d-flex align-items-center gap-2">
              <div className="d-flex align-items-center me-2">
                ...
                <span className="text-muted small">
                  {user?.name || user?.email || t('header.user')}
                  {isAdmin && <span className="badge bg-primary ms-2">{t('header.admin')}</span>}
                </span>
              </div>
              {isAdmin && keycloakAdminUrl && (
                <button
                  onClick={() => window.open(`${keycloakAdminUrl}/admin`, '_blank')}
                  className="btn btn-outline-primary btn-sm"
                >
                  <i className="bi bi-gear-fill me-1"></i>{t('header.keycloak')}
                </button>
              )}
              <button onClick={handleLogout} className="btn btn-danger btn-sm">
                <i className="bi bi-box-arrow-right me-1"></i>{t('header.logout')}
              </button>
            </div>
          </div>
        ...
        >
          <div ref={menuRef}>
            <div className="d-flex align-items-center gap-2 px-4 pt-3 pb-2">
              ...
              <div className="lh-sm">
                <div className="fw-semibold small">{user?.name || user?.email || t('header.user')}</div>
                {user?.email && user?.name && (
                  <div className="text-muted" style={{ fontSize: '11px' }}>{user.email}</div>
                )}
              </div>
              {isAdmin && <span className="badge bg-primary ms-auto" style={{ fontSize: '10px' }}>{t('header.admin')}</span>}
            </div>
            ...
            <div className="d-flex gap-2 px-3 pb-3 pt-1">
              {isAdmin && keycloakAdminUrl && (
                <button
                  onClick={() => { window.open(`${keycloakAdminUrl}/admin`, '_blank'); setMenuOpen(false); }}
